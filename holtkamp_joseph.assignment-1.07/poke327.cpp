...
void encounter_pokemon() {
    int pokemon_level;
    int pokemon_id = rand() % 1092;
    int known_moves;
    int distance = abs(world.cur_idx[dim_x]) + abs(world.cur_idx[dim_y]);
    if (distance <= 200) {
        pokemon_level = 1 > distance / 2 ? 1 : distance / 2;
    } else {
        pokemon_level = 100 < (distance - 200) / 2 ? 100 : (distance - 200) / 2;
    }

    // TODO: stub. This is where i'd build from
}
...
void game_loop() {
    ...
    while (!world.quit) {
        ...
        if (p && (d[dim_x] == 0 || d[dim_x] == MAP_X - 1 || d[dim_y] == 0 ||
                    d[dim_y] == MAP_Y - 1)) {
            leave_map(d);
            d[dim_x] = c->pos[dim_x];
            d[dim_y] = c->pos[dim_y];
